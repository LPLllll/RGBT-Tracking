...
class InerTrack(nn.Module):
    ...
    def forward_head_triple(self, cat_feature, xv, xi, s, gt_score_map=None):
        ...
        if self.head_type == "CORNER":
            ...
            print('%f, %f, %f' % (max_score_rgb, max_score_all, max_score_tir))
            if s is None:
            ...
            #     # m1=torch.zeros_like(max_scores).cuda()
            #     # m2=torch.ones_like(max_scores).cuda()
            #     print('%f, %f, %f'%(self.count, m1, m2))
            #
