...
class Attention(nn.Module):
    ...
    def forward(self, x, flag=False, filter=None):
        ...
        if filter is None:
            ...
            attn = self.softmax_with_policy(attn, filter)
    ...
    def forward_features(self, z, x, kwards):
        ...
        for i, blk in enumerate(self.blocks):
            ...
            if i in self.selection_places:
                ...
                mask_rgb = hard_keep[:, lens_z:lens_x+lens_z].unsqueeze(-1)
                mask_tir = hard_keep[:, -lens_x:].unsqueeze(-1)
                this_mask = torch.cat((mask_rgb, mask_tir), dim=1)
