...
class Attention(nn.Module):
    ...
    def softmax_with_policy(self, attn, filter, eps=1e-6):
        ...
        attn_policy = ((attn_policy.expand(B, 1, N, N) + attn_policy.permute(0,1,3,2).expand(B, 1, N, N))>0).float()
...
class VisionTransformerIner(BaseBackbone):
    ...
    def __init__(self, img_size=224, patch_size=16, in_chans=3, num_classes=1000, embed_dim=768, depth=12,
        ...
        self.norm_tir = norm_layer(embed_dim)
        self.pos_embed_z = nn.Parameter(torch.zeros(1, self.num_patches_template, embed_dim))
        self.pos_embed_x = nn.Parameter(torch.zeros(1, self.num_patches_search, embed_dim))
        self.init_weights(weight_init)
        self.base_pre_norm_rgb = norm_layer(embed_dim)
        ...
        self.base_fuse_block = Block(
                dim=embed_dim, num_heads=num_heads, mlp_ratio=mlp_ratio, qkv_bias=qkv_bias, drop=drop_rate,
                attn_drop=attn_drop_rate, drop_path=0., norm_layer=norm_layer, act_layer=act_layer)
        self.base_fuse_block_rgb = Block(
                dim=embed_dim, num_heads=num_heads, mlp_ratio=mlp_ratio, qkv_bias=qkv_bias, drop=drop_rate,
                attn_drop=attn_drop_rate, drop_path=0., norm_layer=norm_layer, act_layer=act_layer)
        self.base_fuse_block_tir = Block(
                dim=embed_dim, num_heads=num_heads, mlp_ratio=mlp_ratio, qkv_bias=qkv_bias, drop=drop_rate,
                attn_drop=attn_drop_rate, drop_path=0., norm_layer=norm_layer, act_layer=act_layer)
        # self.pre_classifier = pre_classifier(embed=embed_dim)
        ...
        self.selection1=nn.Sequential(
            nn.LayerNorm(self.embed_dim),
            nn.Linear(self.embed_dim, int(self.embed_dim/4)),
            nn.LayerNorm(int(self.embed_dim / 4)),
            nn.Linear(int(self.embed_dim / 4), int(self.embed_dim / 16)),
            nn.LayerNorm(int(self.embed_dim / 16)),
            nn.Linear(int(self.embed_dim / 16), 1),
        )
        self.selection2=nn.Sequential(
            nn.LayerNorm(576),
            nn.Linear(576, 320),
            nn.LayerNorm(320),
            nn.Linear(320, 40),
    ...
    def forward_features(self, z, x, kwards):
        ...
        # s_r = self.selection2(self.selection1(torch.cat(((z_v+z_i)/2, x_v, x_i), dim=1)).permute(0,2,1)).permute(0,2,1)
        # s_r = torch.softmax(s_r, dim=1)

        s_r = None
        # hard_keep = F.gumbel_softmax(s_r, hard=True, dim=-1)[:, :, 0]
        # s_i = self.selection2(self.selection1(torch.cat(((z_v+z_i)/2, x_i), dim=1)).permute(0, 2, 1))
        # all = s_r + s_i
        # s_r = s_r/all
        # s_i = s_i/all
        # if (kwards['change_v'] is not None) and (kwards['change_i'] is not None):
        ...
        # x_v  = torch.cat((x_v[:, :lens_z, :], x_v[:, lens_z:, :] * hard_keep[:, lens_z:].unsqueeze(-1).expand(B, lens_x, self.embed_dim)), dim=1)
        # x_i  = torch.cat((x_i[:, :lens_z, :], x_i[:, lens_z:, :] * (1-hard_keep[:, lens_z:]).unsqueeze(-1).expand(B, lens_x, self.embed_dim)), dim=1)

        x_f = self.base_fuse_block(x_v +x_i).to(torch.float32)# * torch.cat((remained_z, remained), dim=1)
        x_v = self.base_fuse_block_rgb(x_v).to(torch.float32)# * torch.cat((remained_z, remained), dim=1)
        x_i = self.base_fuse_block_tir(x_i).to(torch.float32)# * torch.cat((remained_z, remained), dim=1)
        ...
        aux_dict = {"attn": None, 'mask':s_r}#, 'mask_rgb':original_mask_rgb, 'mask_tir':original_mask_tir
        # return self.norm(x_f), aux_dict
        return self.norm(x_f), self.norm_rgb(x_v), self.norm_tir(x_i), aux_dict
