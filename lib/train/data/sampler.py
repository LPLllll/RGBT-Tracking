...
class TrackingSampler(torch.utils.data.Dataset):
    ...
    def getitem(self):
        ...
        while not valid:
            ...
            # seq_id1, visible1, seq_info_dict1 = self.sample_seq_from_dataset(dataset, is_video_dataset)
            ...
            if is_video_dataset:
                ...
                if self.frame_sample_mode == 'causal':
                    ...
                    while search_frame_ids is None:
                        ...
                        # base_frame_id1 = self._sample_visible_ids(visible1, num_ids=1,
            ...
            try:
                ...
                # opeartion = [0, 1]
                # which2invalid = [1, 2]
                # invalidchoices = 1 #[1, 2] # 0 for specific value; 1 for frame from another video
                #
                # opeartion_ = random.choice(opeartion)
                # which2invalid_ = random.choice(which2invalid)
                # invalidchoice = random.choice(invalidchoices)
                #
                # if opeartion_:
                #     if invalidchoice == 1:
                #         value = np.random.randint(1, 256, 1).item()
                #         # search_frames1 = torch.from_numpy(np.ones((256,256)) * value).unsqueeze(0).repeat(6, 1, 1).unsqueeze(0)
                #         w, h, _ = search_frames[0].shape
                #         search_frames1 = [torch.from_numpy(np.ones((w,h)) * value).unsqueeze(-1).repeat(1, 1, 6).numpy()]
                #     else:
                #         seq_id1, visible1, seq_info_dict1 = self.sample_seq_from_dataset(dataset, is_video_dataset)
                #         search_frame_ids1 = self._sample_visible_ids(visible1, min_id=1, max_id=len(visible), num_ids=1)
                #         search_frames1, search_anno1, meta_obj_test1 = dataset.get_frames(seq_id1, search_frame_ids1,
                #                                                                        seq_info_dict1)
                #
                #     d1 = TensorDict({'template_images': template_frames,
                #                        'template_anno': template_anno['bbox'],
                #                        'template_masks': template_masks,
                #                        'search_images': search_frames1,
                #                        'search_anno': search_anno['bbox'],
                #                        'search_masks': search_masks,
                #                        'dataset': dataset.get_name(),
                #                        'test_class': meta_obj_test.get('object_class_name')})
                #     d1 = self.processing(d1)
                #     if which2invalid_ == 1: # rgb modality is chosen to be invalid
                #         data_vi['visible']['search_images'] = d1['search_images'][:,:3,:,:]
                #         data_vi['visible']['change'] = torch.ones(1) * invalidchoice
                #         # a = 1
                #     else: # tir modality is chosen to be invalid
                #         data_vi['infrared']['search_images'] = d1['search_images'][:,-3:,:,:]
                #         data_vi['infrared']['change'] = torch.ones(1) * invalidchoice
                ###################
