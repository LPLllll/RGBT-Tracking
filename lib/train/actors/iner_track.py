...
class InerTrackActor(BaseActor):
    ...
    def __call__(self, data):
        ...
        # else:
        loss, status = self.compute_losses(out_dict, data['visible'], data['epoch'])
        # loss, status = self.compute_losses_sparse(out_dict, data['visible'])
        return loss, status
    ...
    def forward_pass(self, data):
        ...
        out_dict = self.net(template=[template_img_v, template_img_i],
                            ...
                            return_last_attn=False, epoch=data['epoch'])

        return out_dict

    def compute_losses(self, pred_dict, gt_dict, epoch, return_status=True):
        # gt gaussian map
        ...
        loss = 0.
        pred_boxes_all = pred_dict['pred_boxes']
        for i in range(len(pred_boxes_all)):
            ...
            try:
                giou_loss, iou, area2 = self.objective['giou'](pred_boxes_vec, gt_boxes_vec)  # (BN,4) (BN,4)
            except:
            ...
            if 'score_map' in pred_dict:
                location_loss = self.objective['focal'](pred_dict['score_map'][i], gt_gaussian_maps)
            else:
                location_loss = torch.tensor(0.0, device=l1_loss.device)
            #------------------
            # m = torch.stack(pred_dict['mask'])
            # m=m.squeeze(-1)
            # a, b, c = m.shape[0], m.shape[1], m.shape[2]
            # # if epoch > self.cfg.MODEL.BACKBONE.CE_START:
            # if epoch > 0:
            # # #     if epoch < 1/5 * self.cfg.TRAIN.EPOCH:
            # # #         start=10
            # # #     elif epoch < 1/2 * self.cfg.TRAIN.EPOCH and epoch>= 1/5 * self.cfg.TRAIN.EPOCH:
            # # #         start=8
            # # #     else:
            # # #         start=4
            #     start = 2
            #     label_ratio = []
            #     l = len(self.cfg.MODEL.BACKBONE.MPLT_LOC)
            #     gap = (1 - torch.clamp(area2 * start, min=0.0, max=1.0)) / l
            #     for i in range(l):
            #         label_ratio.append(area2*start + gap * i)
            #     label_ratio.reverse()
            #     label_ratio = torch.stack(label_ratio)
            # #
            # #     # loss_ratio = (pred_dict['ratio'] - area2 * 4).abs().mean()
            # #     loss_ratio = (torch.stack(pred_dict['ratio']) - label_ratio).abs().mean()
            #     m = torch.stack(pred_dict['mask']).squeeze(-1)
            # #     a, b, c, d = m.shape
            # #     loss_ratio = ((((m.sum(dim=2)/c - label_ratio)**2).sum(dim=1)/b).sum())/a
            #     loss_ratio = ((((m.sum(dim=2)/c - label_ratio)**2).sum(dim=1)/b).sum())/a
            # #     m = torch.stack(pred_dict['mask'])
            # #     # a, b, c, d = m.shape
            # #     loss_ratio = m.sum()/(a*b*c)
            # else:
            #     loss_ratio = torch.zeros(1).cuda()

            # mask_label = torch.stack(pred_dict['mask_label'])
            # loss_kl = F.kl_div(F.log_softmax(m.squeeze(-1), dim=-1), torch.softmax(mask_label*1000, dim=-1))

            # weighted sum
            loss += self.loss_weight['giou'] * giou_loss + self.loss_weight['l1'] * l1_loss + self.loss_weight[
                'focal'] * location_loss #+ loss_ratio * 1 #+ loss_kl * 10
        # loss = loss_ratio * 1 + loss_kl * 10
        loss = loss / len(pred_boxes_all)   # not needed for variant with learnable weights
        if return_status:
            # status for log
            mean_iou = iou.detach().mean()
            status = {"Loss/total": loss.item(),
                        # "Loss/ratio": loss_ratio.item(),
