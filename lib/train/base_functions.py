...
from lib.train.dataset import LasHeR, VTUAVST
...
def names2datasets(name_list: list, settings, image_loader):
    ...
    for name in name_list:
        assert name in ["LASOT", "GOT10K_vottrain", "GOT10K_votval", "GOT10K_train_full", "GOT10K_official_val",
                        "COCO17", "VID", "TRACKINGNET", "LasHeR_train", "LasHeR_test", "VTUAV_st","VTUAV_test"]
        if name == "LASOT":
        ...
        if name == "LasHeR_train":
            datasets.append(LasHeR(settings.env.lasher_dir, split='train', image_loader=image_loader))
        if name == "LasHeR_test":
            datasets.append(LasHeR(settings.env.lasher_dir, split='test', image_loader=image_loader))
        if name == "VTUAV_st":
            datasets.append(VTUAVST(settings.env.vtuavst_dir, split='st', image_loader=image_loader))
            if name == "VTUAV_test":
                datasets.append(VTUAVST(settings.env.vtuavsttest_dir, split='test', image_loader=image_loader))
    return datasets
...
def get_optimizer_scheduler(net, cfg):
    ...
    if train_cls:
        ...
        param_dicts = [
            # {"params": [p for n, p in net.named_parameters() if "selection" in n and p.requires_grad]},
            {"params": [p for n, p in net.named_parameters() if "base" in n and p.requires_grad]},
            # {
            #     "params": [p for n, p in net.named_parameters() if
            #                "mplt_fuse_search_rgb" in n or 'mplt_fuse_search_tir' in n and p.requires_grad],
            #     "lr": cfg.TRAIN.LR * cfg.TRAIN.BACKBONE_MULTIPLIER,
            # },
            # {
            #     "params": [p for n, p in net.named_parameters() if
            #                "box_head_rgb" in n or 'box_head_tir' in n and p.requires_grad],
            #     "lr": cfg.TRAIN.LR * cfg.TRAIN.BACKBONE_MULTIPLIER,
            # },
            # {
            #     "params": [p for n, p in net.named_parameters() if
            #                "base_fuse_block_rgb" in n or 'base_fuse_block_tir' in n and p.requires_grad],
            #     "lr": cfg.TRAIN.LR * cfg.TRAIN.BACKBONE_MULTIPLIER,
            # },
            # {
            #     "params": [p for n, p in net.named_parameters() if
            #                "norm_rgb" in n or 'norm_tir' in n and p.requires_grad],
            #     "lr": cfg.TRAIN.LR * cfg.TRAIN.BACKBONE_MULTIPLIER,
            # },
            # {
            #     "params": [p for n, p in net.named_parameters() if
            #                "selection" not in n and 'base' not in n and p.requires_grad],
            #     "lr": cfg.TRAIN.LR * cfg.TRAIN.BACKBONE_MULTIPLIER,
            # },
            # {#and "base" not in n
            #     "params": [p for n, p in net.named_parameters() if
            #                "base_fuse_block" in n and p.requires_grad],
            #     "lr": cfg.TRAIN.LR * cfg.TRAIN.BACKBONE_MULTIPLIER,
            # },
            # #
            # {
            #     "params": [p for n, p in net.named_parameters() if "mplt_fuse_search" in n and p.requires_grad],
            #     "lr": cfg.TRAIN.LR * cfg.TRAIN.BACKBONE_MULTIPLIER,
            # },
            # {
            ...
            # },
            # {
            #     "params": [p for n, p in net.named_parameters() if "backbone" not in n and "box" in n and p.requires_grad],
            #     "lr": cfg.TRAIN.LR * cfg.TRAIN.BACKBONE_MULTIPLIER,
            # },
            {
                "params": [p for n, p in net.named_parameters() if "selection" not in n and "base" not in n and p.requires_grad],
                "lr": cfg.TRAIN.LR * cfg.TRAIN.BACKBONE_MULTIPLIER,
            },
        ]
        # param_dicts = [
