...
# from lib.models.tracker import build_pre_track
...
# from lib.train.actors import PreTrackActor
from lib.train.actors import InerTrackActor
# for import modules
import importlib
...
def run(settings):
    ...
    # Create network
    if settings.script_name == "Iner":
        # net = build_mplt_track(cfg)
        # net = build_base_track(cfg)
        net = build_iner_track(cfg)
    else:
    ...
    if settings.script_name == "Iner":
        ...
        actor = InerTrackActor(net=net, objective=objective, loss_weight=loss_weight, settings=settings, cfg=cfg)
    else:
        # with open('/mnt/fast/nobackup/users/zt00315/Trackers/MPLT-mymain/out.txt', 'a') as f:
        #
