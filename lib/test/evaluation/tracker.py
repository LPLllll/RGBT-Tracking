...
class Tracker:
    ...
    def run_sequence(self, seq, debug=None, run_id=None, name=None):
        ...
        # params.checkpoint = os.path.join(env_settings().save_dir, "%s/checkpoints/train/tracker/%s/MPLTTrack_ep%04d.pth.tar" %
        #                              (name, name, run_id))
    ...
    def _track_sequence(self, tracker, seq, init_info):
        ...
        if isinstance(self.image_v, list):
            # print("{}".format(self.image_v[0].shape[0],))
        ...
        for frame_num, frame_path in enumerate(zip(seq.frames[0][1:], seq.frames[1][1:]), start=1):
            ...
            if len(seq.ground_truth_rect) > 1:
                info['gt_bbox'] = seq.ground_truth_rect[0][frame_num]
