...
from lib.config.Base.config import cfg, update_config_from_file
...
def parameters(yaml_name: str, verson:str, epoch:int):
    ...
    yaml_file = os.path.join(prj_dir, 'experiments/%s/%s.yaml' % ('Base', yaml_name))
    ...
    # params.checkpoint = os.path.join(save_dir, "%s/checkpoints/train/%s/%s/DataParallel_ep%04d.pth.tar" %
