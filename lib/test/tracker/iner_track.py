...
from lib.models.tracker import build_iner_track
...
class InerTrack(BaseTracker):
    def __init__(self, params, dataset_name):
        super(InerTrack, self).__init__(params)
        network = build_iner_track(params.cfg, training=False)
    ...
    def track(self, image, seq_name=None, info: dict = None):
        ...
        # if not os.path.exists('/data/Disk_A/zhangyong/MPLT-mymain-inerselection/results/vis-rgbt/'+self.seq_name):
        #     os.mkdir('/data/Disk_A/zhangyong/MPLT-mymain-inerselection/results/vis-rgbt/'+self.seq_name)
        # plt.imshow(pred_score_map.squeeze(0).squeeze(0).cpu().numpy())
        # plt.axis('off')
        # plt.savefig(fname='/data/Disk_A/zhangyong/MPLT-mymain-inerselection/results/vis-rgbt/'+self.seq_name+'/'+self.seq_name+'-'+str(self.frame_id)+'png', bbox_inches='tight')
